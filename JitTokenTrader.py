...
from JitTradeLib import JitInstrument, JitSignal, jit_load_kline_from_1token, jit_load_kline
from strategy.JitRbreakerStrategy import jit_rbreaker_exit_modified_strategy
from strategy.JitSendOrder import jit_send_order
...
class JitTokenTrader(object):
    ...
    async def test(self):
        s = "binance/btc.usdt-88619183"
        o_info_2, err = await self.account.get_order_use_exchange_oid(s)
        if err:
            log.warning('Get order info failed...', err)
        else:
            # log.info(f'Order information by exchange_oid: {o_info_2}')
    ...
    async def trading(self, loop):
        ...
        while True:
            ...
            for code, instrument in self.instrument_dict.items():
                ...
                signal = jit_send_order(instrument.last_tick, instrument.chart, instrument.position,
